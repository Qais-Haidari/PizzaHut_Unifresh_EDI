...
async function order() {
    ...
    const host = "sftpdev.hungryjacks.com.au";
    const port = "1023";
    const username = "unifreshstaging";
    const password = "IiP8h!BM5$Vg";
    const client = new SFTPClient();
    await client.connect({host,port,username,password});
    const data = await client.listFiles("./po");
    let filename = data[0];
    if (filename == undefined || filename == 'uploaded') {
        ...
        const OrderDate = contents.split(',')[2];
        ...
        let FinalList = [];
        let DeliveryDate;
        for (var i = 0; i < contents.split(',').slice(4, -1).length; i += 6) {
            masterarr.push(contents.split(',').slice(4, -1).slice(i, i + 6))
